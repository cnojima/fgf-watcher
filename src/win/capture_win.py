...
def describe_display_scale(hwnd: int) -> str:
    """Human-readable DPI scaling diagnostic for display_profiles.py's error
    messages - not used for any coordinate math."""
    try:
        dpi = ctypes.windll.user32.GetDpiForWindow(hwnd)
        return f"Windows DPI scaling {round(dpi / 96 * 100)}%"
    except (AttributeError, OSError):
        return "Windows DPI scaling unknown (GetDpiForWindow unavailable)"
